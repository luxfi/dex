...
function disconnectWebSocket(client: LXDexClient): void {
  client.disconnect();
...
class ReconnectingClient {
  ...
  constructor(config: { jsonRpcUrl: string; wsUrl: string }) {
...
class AutoReconnectClient {
  private client: LXDexClient;
  private config: { jsonRpcUrl: string; wsUrl: string };
  ...
  private scheduleReconnect(): void {
    ...
    this.reconnectTimer = setTimeout(async () => {
      ...
      try {
        this.client = new LXDexClient(this.config);
        await this.connect();
      } catch {
        this.scheduleReconnect();
      }
...
class HeartbeatClient {
  ...
  constructor(
    config: { jsonRpcUrl: string; wsUrl: string },
    onDisconnect?: () => void
  ) {
    this.client = new LXDexClient(config);
...
export function useWebSocket(
  ...
  const clientRef = useRef<LXDexClient | null>(null);
  ...
  const connect = useCallback(async () => {
    ...
    try {
      if (!clientRef.current) {
        clientRef.current = new LXDexClient(config);
      }
...
import { OrderBook } from '@luxfi/dex-sdk';
...
class ConnectionMonitor {
  ...
  constructor(client: LXDexClient) {
    this.client = client;
  }
