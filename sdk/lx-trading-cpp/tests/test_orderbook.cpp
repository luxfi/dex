...
TEST_CASE("Orderbook basic operations", "[orderbook]") {
    ...
    SECTION("Add and retrieve levels") {
        ...
        REQUIRE(bids[0].price.to_double() == Catch::Approx(100.0));
        REQUIRE(bids[1].price.to_double() == Catch::Approx(99.0));

        // Asks sorted ascending
        REQUIRE(asks[0].price.to_double() == Catch::Approx(101.0));
        REQUIRE(asks[1].price.to_double() == Catch::Approx(102.0));
    }
    ...
    SECTION("Best bid/ask") {
        ...
        REQUIRE(book.best_bid().value().to_double() == Catch::Approx(100.0));
        REQUIRE(book.best_ask().value().to_double() == Catch::Approx(101.0));
    }
    ...
    SECTION("Mid price and spread") {
        ...
        REQUIRE(book.mid_price().value().to_double() == Catch::Approx(101.0));
        REQUIRE(book.spread().value().to_double() == Catch::Approx(2.0));
        REQUIRE(book.spread_percent().value().to_double() == Catch::Approx(1.98).margin(0.01));
    }
}
...
TEST_CASE("Orderbook VWAP", "[orderbook]") {
    ...
    SECTION("VWAP for small amount") {
        ...
        REQUIRE(vwap->to_double() == Catch::Approx(100.0));
    }
    ...
    SECTION("VWAP across multiple levels") {
        ...
        REQUIRE(vwap->to_double() == Catch::Approx(100.6));
    }
    ...
    SECTION("VWAP for full book") {
        ...
        REQUIRE(vwap->to_double() == Catch::Approx(101.333).margin(0.01));
    }
}
...
TEST_CASE("Orderbook liquidity", "[orderbook]") {
    ...
    SECTION("Bid liquidity") {
        // 1.0 * 100 + 2.0 * 99 = 298
        REQUIRE(book.bid_liquidity().to_double() == Catch::Approx(298.0));
    }

    SECTION("Ask liquidity") {
        // 1.5 * 101 + 2.5 * 102 = 406.5
        REQUIRE(book.ask_liquidity().to_double() == Catch::Approx(406.5));
    }

    SECTION("Depth for specific levels") {
        REQUIRE(book.bid_depth(1).to_double() == Catch::Approx(100.0));
        REQUIRE(book.ask_depth(1).to_double() == Catch::Approx(151.5));
    }
...
TEST_CASE("AggregatedOrderbook", "[orderbook]") {
    ...
    SECTION("Best bid across venues") {
        ...
        REQUIRE(price.to_double() == Catch::Approx(100.0));
    ...
    SECTION("Best ask across venues") {
        ...
        REQUIRE(best.has_value());
        auto [price, venue, qty] = *best;
        REQUIRE(price.to_double() == Catch::Approx(101.0));
