...
func (c *Client) sendOrderBookSnapshot(symbol string) {
	book := c.server.orderBook.GetOrderBookSnapshot()

	// Convert OrderLevel to PriceLevel
	bids := make([]lx.PriceLevel, len(book.Bids))
	for i, bid := range book.Bids {
		bids[i] = lx.PriceLevel{
			Price: bid.Price,
			Size:  bid.Size,
		}
	}

	asks := make([]lx.PriceLevel, len(book.Asks))
	for i, ask := range book.Asks {
		asks[i] = lx.PriceLevel{
			Price: ask.Price,
			Size:  ask.Size,
		}
	}
