...
type LiquidationEngine struct {
	...
	TotalLosses        *big.Int
	InsuranceFundUsed  *big.Int
	LiquidationMetrics *LiquidationMetrics
	LiquidationHistory []*LiquidationOrder
...
type LiquidationQueue struct {
	...
	Orders         []*LiquidationOrder // For testing compatibility
...
func NewLiquidationEngine() *LiquidationEngine {
	return &LiquidationEngine{
		...
		MaintenanceMargin:  initLiquidationMaintenanceMargins(),
		...
		InsuranceFundUsed:  big.NewInt(0),
		LiquidationMetrics: NewLiquidationMetrics(),
		LiquidationHistory: make([]*LiquidationOrder, 0),
...
func (le *LiquidationEngine) calculateLoss(position *MarginPosition) *big.Int {
	// If UnrealizedPnL is positive or zero, no loss
	if position.UnrealizedPnL.Sign() >= 0 {
		return big.NewInt(0)
	}

	// UnrealizedPnL is negative, calculate loss
	// Loss = abs(UnrealizedPnL) - Margin
	absLoss := new(big.Int).Abs(position.UnrealizedPnL)

	// If loss is less than margin, no uncovered loss
	if absLoss.Cmp(position.Margin) <= 0 {
		return big.NewInt(0)
	}

	// Return the uncovered loss (amount exceeding margin)
	return new(big.Int).Sub(absLoss, position.Margin)
}
...
func (fund *InsuranceFund) CanCoverLoss(asset string, loss *big.Int) bool {
	...
	return fund.canCoverLossInternal(asset, loss)
}

func (fund *InsuranceFund) canCoverLossInternal(asset string, loss *big.Int) bool {
...
func (fund *InsuranceFund) CoverLoss(asset string, loss *big.Int, referenceID string) error {
	...
	if !fund.canCoverLossInternal(asset, loss) {
...
func NewLiquidationQueue() *LiquidationQueue {
	return &LiquidationQueue{
		...
		Orders:             make([]*LiquidationOrder, 0), // For testing compatibility
