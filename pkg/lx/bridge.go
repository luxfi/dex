...
func NewEnhancedBridge() *EnhancedBridge {
	return &EnhancedBridge{
		...
		TransferHistory: make([]*BridgeTransfer, 0),
		MultisigWallet: &MultisigBridge{
			RequiredSigs:    2,
			TotalSigners:    3,
...
func (b *EnhancedBridge) InitiateTransfer(
	...
	sourceChainConfig, exists := b.Chains[sourceChain]
	if !exists || !sourceChainConfig.Active {
		return nil, fmt.Errorf("source chain %s is not active", sourceChain)
	}

	destChainConfig, exists := b.Chains[destChain]
	if !exists || !destChainConfig.Active {
		return nil, fmt.Errorf("destination chain %s is not active", destChain)
	}
...
func (b *EnhancedBridge) ValidateTransfer(
	...
	if _, exists := transfer.Validators[validator]; exists {
		return errors.New("validator already validated this transfer")
	}
