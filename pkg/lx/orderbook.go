...
type CircularTradeBuffer struct {
	buffer   []Trade // Changed from fixed array to slice
	head     uint64
	tail     uint64
	size     uint64
	capacity uint64
...
func NewOrderBook(symbol string) *OrderBook {
	ob := &OrderBook{
		...
		Trades: make([]Trade, 0),
		tradesBuffer: &CircularTradeBuffer{
			buffer:   make([]Trade, 1000), // Start with 1000 instead of 100,000
			capacity: 1000,
		},
...
// processMarketOrderLocked for compatibility (assumes lock is already held)
...
func (ob *OrderBook) MatchOrders() []Trade {
	...
	for {
		...
		// Set match type based on whether any order is fully filled
...
func (ob *OrderBook) validateOrder(order *Order) error {
	...
	if order.Type != Market && order.Type != Stop && order.Price <= 0 {
