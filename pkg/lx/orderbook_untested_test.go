...
// Test processMarketOrderLocked function
func TestProcessMarketOrderLocked(t *testing.T) {
	...
	// Process market order (need to lock manually for testing)
	book.mu.Lock()
	tradesCount := book.processMarketOrderLocked(marketBuy)
	book.mu.Unlock()
	...
	book.mu.Lock()
	tradesCount = book.processMarketOrderLocked(marketSell)
	book.mu.Unlock()
