...
//   int dex_orderbook_arena_destroy(void* arena);
//
...
static int call_dex_orderbook_match(
    ...
    uint32_t* num_fills)
{
    return ((lux_dex_orderbook_match_fn)fn)(arena, calldata, out, num_fills);
}

static int call_dex_orderbook_arena_create(void* fn, void** out_arena) {
    return ((lux_dex_orderbook_arena_create_fn)fn)(out_arena);
}

static int call_dex_orderbook_arena_destroy(void* fn, void* arena) {
    return ((lux_dex_orderbook_arena_destroy_fn)fn)(arena);
}
...
// it, only passes it to ArenaDestroy or OrderBookMatch.
//
// ArenaCreate hands ownership to the Go caller; ArenaDestroy releases
// it. The arena lives across many OrderBookMatch calls — that's the
// entire reason it's device-resident: the BookArena holds the
// resting bids/asks for a single book, and successive OrderBookMatch
...
func ArenaCreate() (*OrderBookArena, error) {
	...
	rc := C.call_dex_orderbook_arena_create(h.fnArenaCreate, &raw)
	if rc != 0 {
	...
	return &OrderBookArena{ptr: raw}, nil
}
...
func ArenaDestroy(a *OrderBookArena) error {
	...
	rc := C.call_dex_orderbook_arena_destroy(h.fnArenaDestroy, a.ptr)
	a.ptr = nil
	if rc != 0 {
...
// OrderBookMatch runs one matcher step against the arena. calldata is the
...
func OrderBookMatch(a *OrderBookArena, calldata []byte) (out [OrderBookOutLen]byte, numFills uint32, err error) {
	...
	if a == nil || a.ptr == nil {
		return out, 0, fmt.Errorf("dexvm.OrderBookMatch: arena is nil")
	}
	if len(calldata) != OrderBookCalldataLen {
		return out, 0, fmt.Errorf("dexvm.OrderBookMatch: calldata len=%d, want %d",
			len(calldata), OrderBookCalldataLen)
	}
	...
	if rc != 0 {
		return out, 0, fmt.Errorf("dexvm.OrderBookMatch: %s plugin returned rc=%d",
			h.backend, int(rc))
	}
