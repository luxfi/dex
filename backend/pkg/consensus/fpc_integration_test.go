...
func TestFPCDAGOrderBookCreation(t *testing.T) {
	...
	assert.True(t, dob.luxConfig.Enable)
	assert.Equal(t, 0.55, dob.luxConfig.ThetaMin)
	assert.Equal(t, 0.65, dob.luxConfig.ThetaMax)
	assert.Equal(t, 256, dob.luxConfig.VoteLimitPerBlock)
...
func TestFPCConsensusRounds(t *testing.T) {
	...
	for round := 1; round <= 10; round++ {
		...
		expectedThreshold := dob.luxConfig.ThetaMin +
			(dob.luxConfig.ThetaMax-dob.luxConfig.ThetaMin)*float64(round)/10.0
		if round > 10 {
			expectedThreshold = dob.luxConfig.ThetaMax
		}
	...
	require.NoError(t, err)
	assert.Equal(t, dob.luxConfig.ThetaMax, dob.voteThreshold)
}
